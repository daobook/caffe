...
class InstallCMakeLibs(install_lib):
    ...
    def run(self):
        ...
        libs = [os.path.join(root, _lib) for root, _, files in
                ...
                _lib.startswith("libcaffe")
                and not (_lib.startswith("python") or _lib.startswith("_caffe") or _lib.endswith(".a"))]
...
class BuildCMakeExt(build_ext):
    ...
    def build_cmake(self, ext: Extension):
        ...
        pyd_path = [os.path.join(root, _pyd) for root, _, files in
                    ...
                    os.path.splitext(_pyd)[0].startswith('_caffe') and
                    os.path.splitext(_pyd)[-1] in [".pyd", ".so", ".dylib"]][0]
...
def find_version():
    ...
    version_prefix = re.findall(r"CAFFE_TARGET_VERSION \"(.+)\" C", version_file)
